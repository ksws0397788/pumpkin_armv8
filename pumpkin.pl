...
&pumpkin_init();
&cmd_parsing();
&create_dir($pumpkin_path_hash{'working_temp_dir'});
...
sub pumpkin_init
{
        ...
        {
                die "$key doesn't exist at $pumpkin_path_hash{$key}"
...
sub cmd_parsing
{
        ...
        foreach my $current_arg (@ARGV)
        {
                ...
                elsif($current_arg =~ /-(?<mode>behavioral|synthesis|implementation)/i)
                {
